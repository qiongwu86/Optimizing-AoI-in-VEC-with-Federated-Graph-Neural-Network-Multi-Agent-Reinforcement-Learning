...
from Compare5_EnvironmentGCN_SAC_new_global import  VEC_env
...
def get_args():
    ...
    parser.add_argument("--time-steps", type=int, default=510000, help="number of time steps")
    # 一个地图最多env.n个agents，用户可以定义min(env.n,num-adversaries)个敌人，剩下的是好的agent
    parser.add_argument("--num-adversaries", type=int, default=1, help="number of adversaries")
    # Core training parameters
    parser.add_argument("--type", type=int, default = 1, help="0:MADDPG, 1 : IDMADDPG")
    ...
    # Checkpointing
    parser.add_argument("--save-dir", type=str, default="./save/Compare5_MyCode/model", help="directory in which training state and model should be saved")
    parser.add_argument("--save-rate", type=int, default=100000, help="save model once every time this many episodes are completed")
...
def main():
    ...
    env = VEC_env(lane = num_lane,vehGenRate = args.n_agents,
                  ...
                  DRL_model= None,GCN_batchsize = GCN_batchsize,p = p,maxP =maxP,slideW = slideW,w = w,k=k,
    ...
    for episode in range(TotalEpi):
        ...
        for simSlot in range(args.max_episode_len):
            ...
            AOE_T.append(AOI)
            print("episode:", epi-1 , "slot : ", simSlot, ", AOI : ", AOI)
            writer.add_scalar('AOI', AOI, simSlot)
