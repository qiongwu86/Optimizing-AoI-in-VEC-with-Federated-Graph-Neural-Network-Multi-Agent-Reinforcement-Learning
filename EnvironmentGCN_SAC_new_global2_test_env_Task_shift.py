...
class VEC_veh:
    def __init__(self,lane,VehSpeed,slotT,rsuX,rsuY,dis,taskGenRate,TaskSize,TransmissionRange,node,model,record_veh_num,reward,seed=123,is_test_mode=False,burst_task_probability=None,test_task_size_min=1.0,test_task_size_max=100.0):
        super().__init__()
        self.idx = uuid.uuid4()
        ...
        self.burst_task_probability = burst_task_probability  # 突发任务触发概率（每个slot）
        self.test_task_size_min = test_task_size_min  # 测试模式任务大小最小值 (MB)
        self.test_task_size_max = test_task_size_max  # 测试模式任务大小最大值 (MB)
        self.training_task_size_min = 0.1  # 训练模式任务大小最小值 (MB)
        self.training_task_size_max = TaskSize  # 训练模式任务大小最大值 (MB)
        # 根据模式选择任务大小范围
        if self.is_test_mode:
            self.TaskSize = np.random.uniform(self.test_task_size_min, self.test_task_size_max) * MB
        else:
            self.TaskSize = np.random.uniform(self.training_task_size_min, self.training_task_size_max) * MB
    ...
    def GenerateTask(self, current_slot=0, force_burst=False):
        """
        生成任务
        Args:
            current_slot: 当前时间槽（已废弃，保留用于兼容性）
            force_burst: 是否强制突发任务（所有车辆同时生成任务）
        """
        self.Transimission_AOI = 0
        
        # 生成任务的条件：正常到达时间 OR 强制突发（突发任务由环境统一控制）
        if self.GenTaskNextSlot <= 0 or force_burst:
            # 使用泊松分布生成下次任务到达时间
            self.GenTaskNextSlot = np.random.poisson(lam=self.taskGenRate)
...
class VEC_env:
    ...
    def __init__(self,lane,vehGenRate,slotT,VehSpeed,rsuW,TaskGenRate,TaskSize,TransmissionRange,gridW,node_feature_size,out_feature
                 ,p,DRL_model,GCN_batchsize,maxP,slideW,w,k,penalty_factor,powerFactor,param_noise_var,GCN_factor, critic_noise_var , rateDecay,GCN_critic,reward_scale,penalty_multiplier=1.0,plot=False,is_test_mode=False,burst_task_interval=None,burst_task_probability=None,test_task_size_min=1.0,test_task_size_max=100.0):
        super().__init__()
        self.episode = 0
        self.slotT = slotT
        self.lane = lane
        ...
        # 测试模式参数
        self.is_test_mode = is_test_mode
        # 突发任务配置：随机触发概率（每个slot触发突发任务的概率）
        # 优先使用 burst_task_probability，如果没有则从 burst_task_interval 转换
        if burst_task_probability is not None:
            self.burst_task_probability = burst_task_probability
        elif burst_task_interval is not None:
            if burst_task_interval > 1.0:
                # 传入的是间隔，转换为概率（例如：间隔100 -> 概率0.01）
                self.burst_task_probability = 1.0 / burst_task_interval
            else:
                # 传入的是概率，直接使用
                self.burst_task_probability = burst_task_interval
        else:
            self.burst_task_probability = None
        self.test_task_size_min = test_task_size_min  # 测试模式任务大小最小值 (MB)
        self.test_task_size_max = test_task_size_max  # 测试模式任务大小最大值 (MB)
        self.current_burst_trigger = False  # 当前slot是否触发突发任务
        # GBB -----------------------------------------------------------------------------------------
        self.p = p
        self.node_feature_size = node_feature_size
        ...
        for lane_idx in range(self.lane):
            if self.GenNextSlot[lane_idx] <= 0:
                ...
                veh = VEC_veh(lane_idx, Speed, self.slotT, self.rsuX, self.rsuY, dis, self.TaskGenRate, self.TaskSize,
                              ...
                              reward=self.reward, is_test_mode=self.is_test_mode, 
                              burst_task_probability=self.burst_task_probability,
                              test_task_size_min=self.test_task_size_min,
                              test_task_size_max=self.test_task_size_max)
    ...
    def updateVehLoc(self):
        # 在测试模式下，随机决定是否触发突发任务（所有车辆同时生成任务）
        force_burst = False
        if self.is_test_mode and self.burst_task_probability is not None:
            # 每个slot随机决定是否触发突发任务
            if np.random.random() < self.burst_task_probability:
                force_burst = True
                self.current_burst_trigger = True
            else:
                self.current_burst_trigger = False
        ...
        for veh in self.Vehicle:
            ...
            veh.updateChannel()
            veh.GenerateTask(force_burst=force_burst)  # 传递突发任务标志
