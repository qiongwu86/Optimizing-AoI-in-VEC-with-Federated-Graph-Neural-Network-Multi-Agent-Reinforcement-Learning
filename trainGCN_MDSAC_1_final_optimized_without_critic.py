...
from EnvironmentGCN_SAC_new_global2_withou_critic import  VEC_env
    ...
    for episode in range(TotalEpi):
        with memory_optimizer.memory_context(f"Episode {episode}"):
            ...
            for simSlot in range(int(TotalSlot)):
                ...
                # 使用内存监控的训练步骤
                AOI, mean_veh_reward ,penalty,  n_veh, destroyAOI,  gnnloss = memory_optimizer.monitor_training_step(
                    env.step
                )
