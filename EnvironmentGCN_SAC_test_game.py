...
class VEC_veh:
    def __init__(self,lane,VehSpeed,slotT,rsuX,rsuY,dis,taskGenRate,TaskSize,TransmissionRange,node,model,record_veh_num,reward,seed=123):
        ...
        # Game Theory不使用DRL模型，model参数保留用于兼容性但忽略
        
        # Game Theory算法中，action由博弈论算法决定（不是DRL）
        self.action = 0
        self.action_pre = 0
    ...
    def reset(self, model=None):
        ...
        # Game Theory不使用DRL模型，action由博弈论算法决定
        self.action = 0
        self.action_pre = 0
        self.last_action = self.action
...
class VEC_env:
    ...
    def __init__(self,lane,vehGenRate,slotT,VehSpeed,rsuW,TaskGenRate,TaskSize,TransmissionRange,gridW,node_feature_size,out_feature
        ...
        self.meanAOI = 0.0
        # Game Theory不使用DRL模型，保留参数用于兼容性但设为None
        self.DRL_model = None
        ...
        # Game Theory不使用GCN和DRL模型，删除相关初始化
    ...
    def updateSINR(self):
        ...
        P_all = [1 - veh.p * self.TaskRate for veh in self.Vehicle]
        ...
        for veh in self.Vehicle:
            ...
            # s = veh.updatestate()
            # veh.s0 = s
            if len(self.Vehicle) == 1 :
                cumulative_product = 0
            else:
                cumulative_product = 1
                for P_i in range(len(P_all)):
                    if veh_i == P_i:
                        continue
                    else:
                        cumulative_product *= P_all[P_i]

            veh.collectAOI()
            # print(veh.AOI_list )
            # print(np.mean(veh.AOI_list))
            if len(veh.AOI_list ) == 0:
                veh.p = max( min( (0)/(2 * 0.7 * (1-cumulative_product)),0.99),0.01)
            elif len(veh.AOI_list) == 1:
                veh.p = max(min((np.mean(veh.AOI_list))/(self.reward+ 0.01) /(2 * 0.7 * (1-cumulative_product)),0.99),0.01)
                # veh.p = max(min((np.mean(veh.AOI_list))/(veh.AOI_list[0]+0.01)/(2 * 0.7 * (1-cumulative_product)),0.99),0.01)
            else:
                veh.p = max(min((np.mean(veh.AOI_list) - np.mean(veh.AOI_list[1:]))/(self.reward + 0.01) / (2 * 0.7 * (1 - cumulative_product)),0.99),0.01)

            # print(veh.p)


            veh.Translate = False
            veh.action = veh.p * self.maxP
    ...
    def updateGrapth(self,V2Vlink_indx):
        ...
        for veh in self.Vehicle:
            ...
            # Game Theory不使用DRL，删除DRL相关特征
            self.node[node_index][1] += 0  # 保留结构但设为0
            self.node[node_index][2] += 0  # 保留结构但设为0
            self.node[node_index][3] += 0  # 保留结构但设为0
            self.node[node_index][4] += 0  # 保留结构但设为0
            mask[node_index] = 1
    ...
    def updateMemory(self,Channel_list):
        ...
        for veh in self.Vehicle:
            ...
            # Game Theory环境不需要更新DRL模型的内存
            # 只更新状态信息
            s1 = veh.updatestate(len(self.Vehicle))
            # s1 = self.processState(s1)
            
            # 初始化done属性（如果不存在）
            if not hasattr(veh, 'done'):
                veh.done = False
            
            if (veh.x + self.slotT * veh.v) >= self.maxRoadLen:
                veh.done = True

            # Game Theory不使用DRL模型，所以不需要调用veh.model.memory.Addremember
            # 如果veh.model存在（理论上不应该存在），才更新DRL内存
            if veh.model is not None:
                # 确保action_pre存在，如果不存在则使用action
                action_pre = veh.action_pre if hasattr(veh, 'action_pre') else veh.action
                veh.model.memory.Addremember(np.concatenate((veh.s0, [action_pre], [-veh.reward], s1, [veh.done])))
    ...
    def step(self):
        ...
        for veh in self.Vehicle:
            # 确保veh.reward存在（updateMemory应该已经设置了它）
            # 如果不存在，使用系统奖励作为后备
            if not hasattr(veh, 'reward'):
                veh.reward = self.reward + self.penalty
