...
class VEC_veh:
    def __init__(self,lane,VehSpeed,slotT,rsuX,rsuY,dis,taskGenRate,TaskSize,TransmissionRange,node,model,record_veh_num,reward,seed=123):
        ...
        # CSI陈旧/噪声相关：初始化时先设置为None，后续通过set_csi_params设置
        self.csi_lag_slots = None
        self.csi_noise_variance_linear = None
        self.enable_csi_staleness = False
        # 环形缓冲区存储历史真实channel值（用于CSI滞后）
        self.channel_history = deque(maxlen=100)  # 最多存储100个历史值
        self.channel_visible = self.channel  # 可见CSI（用于调度），初始化为真实channel
        self.sinr = 0
        self.lastSINR = 0
        self.rho1 = 0
        # task
    ...
    def set_csi_params(self, lag_slots, noise_variance_linear, enable):
        """设置CSI陈旧/噪声参数"""
        self.csi_lag_slots = lag_slots
        self.csi_noise_variance_linear = noise_variance_linear
        self.enable_csi_staleness = enable
    ...
    def updateV2I(self):
        ...
        # CSI陈旧/噪声处理
        if self.enable_csi_staleness:
            # 将真实channel保存到历史记录
            self.channel_history.append(self.channel)
            
            # 计算可见CSI（滞后的channel + 噪声）
            # 获取滞后的channel值：如果历史记录足够，使用滞后lag_slots的值；否则使用最早的值
            if len(self.channel_history) > self.csi_lag_slots:
                # 获取滞后的channel值（倒数第lag_slots+1个，因为刚append了当前值）
                H_delayed = list(self.channel_history)[-self.csi_lag_slots - 1]
            elif len(self.channel_history) > 0:
                # 如果历史记录不够，使用最早的值
                H_delayed = self.channel_history[0]
            else:
                # 如果历史记录为空（不应该发生），使用当前channel
                H_delayed = self.channel
            
            # 添加估计噪声（复高斯噪声，这里简化为实值噪声）
            if self.csi_noise_variance_linear > 0:
                noise = np.random.normal(0, np.sqrt(self.csi_noise_variance_linear / 2))
                # 对于实值channel，直接添加噪声
                H_hat = H_delayed + noise
                # 确保H_hat不为负
                H_hat = max(H_hat, 1e-20)
            else:
                H_hat = H_delayed
            
            # 更新可见CSI（用于调度）
            self.channel_visible = H_hat
        else:
            # 如果未启用CSI陈旧功能，可见CSI等于真实channel
            self.channel_history.append(self.channel)
            self.channel_visible = self.channel
    def updateV2V(self):
        pass
    ...
    def updatestate(self,n_v):
        # 注意：channel已经在updateV2I中更新，这里使用channel_visible（用于调度决策）
        # 如果channel_visible未初始化，使用channel
        channel_for_state = self.channel_visible if hasattr(self, 'channel_visible') else self.channel
        self.meanAoI = 0
        self.maxAOI = 0
        for task in self.queue:
        ...
        if np.isnan(channel_for_state) or np.isnan(self.lastSINR):
                print(channel_for_state, self.maxAOI, self.env_AOI, self.lastSINR, self.dis, TaskSize / MB, n_v)
                import pdb
                pdb.set_trace()
        return self.process_state(np.concatenate(([channel_for_state * 1e12 ],
                                                  [self.maxAOI],
                                                  [self.env_AOI],
                                                  [self.dis],
                                                  [TaskSize/MB],
...
class VEC_env:
    ...
    def __init__(self,lane,vehGenRate,slotT,VehSpeed,rsuW,TaskGenRate,TaskSize,TransmissionRange,gridW,node_feature_size,out_feature
                 ,p,DRL_model,GCN_batchsize,maxP,slideW,w,k,penalty_factor,powerFactor,param_noise_var,GCN_factor, critic_noise_var , rateDecay,GCN_critic,reward_scale,penalty_multiplier=1.0,csi_lag_slots=5,csi_noise_variance_db=-10,enable_csi_staleness=True,plot=False):
        super().__init__()
        self.episode = 0
        self.slotT = slotT
        self.lane = lane
        ...
        # CSI陈旧/噪声参数
        self.csi_lag_slots = csi_lag_slots  # 滞后时间（slot数）
        self.csi_noise_variance_db = csi_noise_variance_db  # 噪声方差（dB）
        self.enable_csi_staleness = enable_csi_staleness  # 是否启用CSI陈旧功能
        # 将噪声方差从dB转换为线性值
        self.csi_noise_variance_linear = np.power(10.0, csi_noise_variance_db / 10.0) if enable_csi_staleness else 0.0
        self.slot = 0
    ...
    def generateVeh(self):
        ...
        for lane_idx in range(self.lane):
            if self.GenNextSlot[lane_idx] <= 0:
                ...
                # 设置CSI陈旧/噪声参数
                veh.set_csi_params(self.csi_lag_slots, self.csi_noise_variance_linear, self.enable_csi_staleness)
