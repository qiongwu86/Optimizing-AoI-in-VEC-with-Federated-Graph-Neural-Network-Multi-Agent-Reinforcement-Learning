...
class GCNMemory:
    ...
    def Addremember(self, TraingcnData,r,TraingcnData_,G_data,G_data_):
        # 确保数据是tensor格式
        if not isinstance(TraingcnData, torch.Tensor):
            TraingcnData = torch.tensor(TraingcnData, dtype=torch.float32)
        if not isinstance(TraingcnData_, torch.Tensor):
            TraingcnData_ = torch.tensor(TraingcnData_, dtype=torch.float32)
        
        # 检查数据是否包含NaN或无穷大值
        if (torch.isnan(TraingcnData).any() or torch.isinf(TraingcnData).any() or
            torch.isnan(TraingcnData_).any() or torch.isinf(TraingcnData_).any() or
            np.isnan(r) or np.isinf(r)):
            print("Warning: NaN or Inf detected in GCN data, skipping storage")
            import pdb
            return  # 不存储坏数据
        ...
        # 如果记忆超过设定的大小，删除最早的记忆
        if len(self.memory_gcn) > self.memory_size:
            self.memory_gcn.pop(0)
            self.memory_gcn_.pop(0)
            self.G_data.pop(0)
            self.G_data_.pop(0)
            self.memory_r.pop(0)
...
class My_GCN(torch.nn.Module):
    def __init__(self,Input_feauture_size,output_featrue_size):
        ...
        # 优化：减少到3层GNN，使用LeakyReLU替代Tanh，添加残差连接
        self.conv1 = GCNConv(Input_feauture_size, 64)  # 减少隐藏层大小
        self.conv2 = GCNConv(64, 32)  # 减少隐藏层大小
        self.conv3 = GCNConv(32, output_featrue_size)  # 最后一层
        
        # 添加残差连接的线性层
        self.residual1 = nn.Linear(Input_feauture_size, 64)
        self.residual2 = nn.Linear(64, 32)
        
        # 使用LeakyReLU替代Tanh，避免梯度消失
    ...
    def forward(self, x, edge_index):
        ...
        out = self.activation(out)  # 使用LeakyReLU而不是Tanh
        
        # 添加输出归一化，将输出控制在0-1之间
        # 使用sigmoid函数将输出限制在(0,1)范围内
        out = torch.sigmoid(out)
        
        # 检查输出范围，如果超出预期则打印警告
        if torch.max(out) > 1.0 or torch.min(out) < 0.0:
            print(f"⚠️⚠️⚠️[DEBUG] GCN output out of range: min={torch.min(out)}, max={torch.max(out)}")
            import pdb
            pdb.set_trace()
    ...
    def GCN_train_from_torch(self,Traindata_node,r,Traindata_node_):
        # 检查输入数据是否有NaN或无穷大值
        if torch.isnan(Traindata_node).any() or torch.isinf(Traindata_node).any():
            print("⚠️ ⚠️ Warning: NaN or Inf detected in Traindata_node, skipping GCN training")
            import pdb
            pdb.set_trace()
            return torch.tensor(0.0), torch.tensor(0.0)
        if torch.isnan(r).any() or torch.isinf(r).any():
            print("⚠️ ⚠️ Warning: NaN or Inf detected in reward, skipping GCN training") 
            import pdb
            pdb.set_trace()
            return torch.tensor(0.0), torch.tensor(0.0)
        if torch.isnan(Traindata_node_).any() or torch.isinf(Traindata_node_).any():
            print("⚠️ ⚠️ Warning: NaN or Inf detected in Traindata_node_, skipping GCN training")
            import pdb
            pdb.set_trace()
            return torch.tensor(0.0), torch.tensor(0.0)
        ...
        # 检查gcn_loss是否有异常值
        if torch.isnan(gcn_loss) or torch.isinf(gcn_loss):
            print("⚠️ ⚠️ Warning: NaN or Inf detected in gcn_loss, skipping GCN training")  
            import pdb
            pdb.set_trace()
            return torch.tensor(0.0), torch.tensor(0.0)
        ...
        # 添加梯度裁剪
        torch.nn.utils.clip_grad_norm_(self.GCN_critic.parameters(), max_norm=1.0)
        ...
        if torch.isnan(q).any() or torch.isinf(q).any():
            print("⚠️ ⚠️ Warning: NaN or Inf detected in Q values, skipping critic training")
            import pdb
            pdb.set_trace()
            return gcn_loss, torch.tensor(0.0)
        if torch.isnan(target_y).any() or torch.isinf(target_y).any():
            print("⚠️ ⚠️ Warning: NaN or Inf detected in target_y, skipping critic training")
            import pdb
            pdb.set_trace()
            return gcn_loss, torch.tensor(0.0)
        ...
        if torch.isnan(critic_loss) or torch.isinf(critic_loss):
            print("⚠️ ⚠️ Warning: NaN or Inf detected in critic_loss, skipping critic training")
            import pdb
            pdb.set_trace()
            return gcn_loss, torch.tensor(0.0)
    ...
    def train(self,num_nodes,GCN_critic,node_feature_size,out_feature_size,reward_scale):
        ...
        self.gcn_optimizer = torch.optim.Adam(
            [{"params": self.conv1.parameters()},
             ...
             {"params": self.residual1.parameters()},
             {"params": self.residual2.parameters()}], lr=1e-3
        )
    ...
    def NSLoss(self,data,out_feature):
        ...
        # 添加数值稳定性处理
        sigmoid_val = torch.sigmoid(torch.sum(torch.mul(embs_node, embs_NeiborNode), 1))
        # 防止log(0)的情况
        sigmoid_val = torch.clamp(sigmoid_val, min=1e-8, max=1.0)
        log_target = torch.log(sigmoid_val)  # self.weights[label] 生成另一个节点的信息
        ...
        # 添加数值稳定性处理
        sigmoid_sampled = torch.sigmoid(torch.bmm(noise, embs_node.unsqueeze(2)))
        sigmoid_sampled = torch.clamp(sigmoid_sampled, min=1e-8, max=1.0)
        sum_log_sampled = torch.sum(
             torch.log(sigmoid_sampled), 1
         ).squeeze()
        #
...
class New_task:
    def __init__(self,TaskSize):
        ...
        self.AOI = 0
    def __del__(self):
        # Note: 'del self' is unnecessary and should not be used in __del__
        pass
...
class VEC_veh:
    def __init__(self,lane,VehSpeed,slotT,rsuX,rsuY,dis,taskGenRate,TaskSize,TransmissionRange,node,model,record_veh_num,reward,seed=123):
        ...
        self.rsuLoc = np.array([rsuX,rsuY,Hight_RSU])  # 初始使用第一个RSU的位置
        self.lamda = f_c/v_c # wavelength                     #
        # self.neiborVeh = getneiborVeh(self.x, )            # 邻居车辆
        self.dis = dis
        # ==================== 多RSU支持 ====================
        self.current_rsu = 1  # 当前连接的RSU编号（从1开始，支持多个RSU）
        self.rsuLocs = {}  # 存储所有RSU的位置字典 {rsu_id: [x, y, z]}
        self.dis_rsus = {}  # 存储到所有RSU的距离字典 {rsu_id: distance}
        self.sigma2 = 1e-9
        self.node = node
...
class VEC_env:
    ...
    def __init__(self,lane,vehGenRate,slotT,VehSpeed,rsuW,TaskGenRate,TaskSize,TransmissionRange,gridW,node_feature_size,out_feature
                 ,p,DRL_model,GCN_batchsize,maxP,slideW,w,k,penalty_factor,powerFactor,param_noise_var,GCN_factor, critic_noise_var , rateDecay,GCN_critic,reward_scale,penalty_multiplier=1.0,plot=False,DRL_model_RSU1=None,DRL_model_RSU2=None,num_rsu=2,DRL_models=None):
        super().__init__()
        self.episode = 0
        self.slotT = slotT
        self.lane = lane
        ...
        # ==================== 多RSU配置（支持2-6个RSU）====================
        # 如果提供了DRL_models列表，使用它；否则使用向后兼容的方式
        if DRL_models is not None and len(DRL_models) > 0:
            self.num_rsu = len(DRL_models)
            self.DRL_models = DRL_models
        else:
            # 向后兼容：如果没有提供DRL_models，使用num_rsu参数或默认值
            if num_rsu is not None and num_rsu >= 1:
                self.num_rsu = num_rsu
            else:
                # 检查是否提供了RSU1和RSU2模型（向后兼容）
                if DRL_model_RSU1 is not None or DRL_model_RSU2 is not None:
                    self.num_rsu = 2
                else:
                    self.num_rsu = 2  # 默认值
            ...
            for i in range(self.num_rsu):
                if i == 0 and DRL_model_RSU1 is not None:
                    self.DRL_models.append(DRL_model_RSU1)
                elif i == 1 and DRL_model_RSU2 is not None:
                    self.DRL_models.append(DRL_model_RSU2)
                else:
                    # 使用默认模型
                    self.DRL_models.append(DRL_model)
        
        # 单个RSU覆盖范围是 2 * rsuW，RSU不重叠
        # 第i个RSU（从1开始）：
        #   - 位置：x = (2*i - 1) * rsuW
        #   - 覆盖范围：[(2*(i-1))*rsuW, (2*i)*rsuW]
        # 道路总长度：2 * num_rsu * rsuW
        self.rsuW = rsuW
        self.maxRoadLen = 2 * self.num_rsu * rsuW
        
        # 存储所有RSU的位置和覆盖范围信息
        self.rsu_positions = []  # 存储每个RSU的[x, y]位置
        self.rsu_coverage = []  # 存储每个RSU的覆盖范围[(start, end), ...]
        self.rsu_vehicles = [[] for _ in range(self.num_rsu)]  # 存储每个RSU覆盖范围内的车辆
        
        # 动态计算每个RSU的位置和覆盖范围
        for i in range(self.num_rsu):
            rsu_idx = i + 1  # RSU编号从1开始
            rsu_x = (2 * rsu_idx - 1) * rsuW
            rsu_y = 0
            coverage_start = 2 * i * rsuW
            coverage_end = 2 * (i + 1) * rsuW
            
            self.rsu_positions.append([rsu_x, rsu_y])
            self.rsu_coverage.append((coverage_start, coverage_end))
        
        # 为了兼容性，保留原来的rsuX和rsuY（使用第一个RSU的位置）
        # 注意：这些属性主要用于一些旧代码（如resetVeh和plot函数）
        # 核心逻辑应使用 self.rsu_positions 列表访问所有RSU信息
        if self.num_rsu >= 1:
            self.rsuX = self.rsu_positions[0][0]
            self.rsuY = self.rsu_positions[0][1]
        else:
            self.rsuX = None
            self.rsuY = None

        self.meanAOI = 0.0
        self.DRL_model = DRL_model  # 保留用于兼容性
        ...
        # 节点数量基于道路长度和车道数
        num_nodes = self.lane * (int(self.maxRoadLen/self.gridW))
        self.node = torch.tensor(np.zeros((num_nodes, self.node_feature_size)), dtype=torch.float)
        ...
        self.GCN_model.train(num_nodes, GCN_critic, self.node_feature_size, self.out_feature_size, reward_scale)
    ...
    def generateVeh(self):
        ...
        for lane_idx in range(self.lane):
            if self.GenNextSlot[lane_idx] <= 0:
                ...
                # 车辆从x=0开始生成，首先进入第一个RSU的覆盖范围
                # 计算到所有RSU的距离
                veh_x_init = 0
                veh_y_init = 3.5 / 2 + lane_idx * 3.5
                
                # 找到车辆所在的RSU（应该是在第一个RSU的覆盖范围内）
                current_rsu_idx = 0  # 默认是第一个RSU
                for i, (coverage_start, coverage_end) in enumerate(self.rsu_coverage):
                    if coverage_start <= veh_x_init < coverage_end:
                        current_rsu_idx = i
                        break
                
                # 计算到当前RSU的距离
                rsu_pos = self.rsu_positions[current_rsu_idx]
                dis = np.sqrt((rsu_pos[0] - veh_x_init) ** 2 + (rsu_pos[1] - veh_y_init) ** 2 + Hight_RSU ** 2)
                
                node = int(lane_idx * int(self.maxRoadLen / self.gridW) + veh_x_init // self.gridW)
                # generate new vehicle with current RSU's model
                current_rsu_model = self.DRL_models[current_rsu_idx]
                veh = VEC_veh(lane_idx, Speed, self.slotT, rsu_pos[0], rsu_pos[1], dis, self.TaskGenRate, self.TaskSize,
                              self.TransmissionRange, node, copy.deepcopy(current_rsu_model), self.record_veh_num,
                              reward=self.reward)
                
                # 设置车辆的多RSU信息
                veh.current_rsu = current_rsu_idx + 1  # RSU编号从1开始
                veh.rsuLocs = {}  # 存储所有RSU的位置
                veh.dis_rsus = {}  # 存储到所有RSU的距离
                
                # 计算到所有RSU的距离
                for i, rsu_pos in enumerate(self.rsu_positions):
                    rsu_idx = i + 1
                    dis_rsu = np.sqrt((rsu_pos[0] - veh_x_init) ** 2 + (rsu_pos[1] - veh_y_init) ** 2 + Hight_RSU ** 2)
                    veh.rsuLocs[rsu_idx] = np.array([rsu_pos[0], rsu_pos[1], Hight_RSU])
                    veh.dis_rsus[rsu_idx] = dis_rsu
    ...
    def updateVehLoc(self):
        for veh in self.Vehicle:
            ...
            # ==================== 多RSU支持：按覆盖范围判断（不重叠） ====================
            # 计算到所有RSU的距离（用于通信计算）
            if not hasattr(veh, 'rsuLocs') or veh.rsuLocs is None:
                veh.rsuLocs = {}
                veh.dis_rsus = {}
            ...
            for i, rsu_pos in enumerate(self.rsu_positions):
                rsu_idx = i + 1
                dis_rsu = np.sqrt((rsu_pos[0] - veh.x) ** 2 + (rsu_pos[1] - veh.y) ** 2 + Hight_RSU**2)
                veh.rsuLocs[rsu_idx] = np.array([rsu_pos[0], rsu_pos[1], Hight_RSU])
                veh.dis_rsus[rsu_idx] = dis_rsu
            
            # 根据车辆x坐标判断在哪个RSU的覆盖范围内（不重叠，按顺序）
            old_rsu = veh.current_rsu
            new_rsu = None
            current_rsu_idx = None
            
            # 查找车辆当前所在的RSU覆盖范围
            for i, (coverage_start, coverage_end) in enumerate(self.rsu_coverage):
                if coverage_start <= veh.x < coverage_end:
                    current_rsu_idx = i
                    new_rsu = i + 1  # RSU编号从1开始
                    veh.current_rsu = new_rsu
                    break
            
            if current_rsu_idx is not None:
                # 车辆在某个RSU的覆盖范围内
                veh.dis = veh.dis_rsus[new_rsu]
                veh.rsuLoc = veh.rsuLocs[new_rsu]
            else:
                # 车辆不在任何RSU覆盖范围内，保持当前RSU（如果之前有连接）
                if veh.current_rsu is not None and veh.current_rsu in veh.dis_rsus:
                    veh.dis = veh.dis_rsus[veh.current_rsu]
                    veh.rsuLoc = veh.rsuLocs[veh.current_rsu]
                else:
                    # 默认使用第一个RSU（车辆刚生成，还未进入任何RSU）
                    veh.current_rsu = 1
                    new_rsu = 1
                    veh.dis = veh.dis_rsus.get(1, 0)
                    veh.rsuLoc = veh.rsuLocs.get(1, np.array([0, 0, Hight_RSU]))
            
            # 如果车辆切换了RSU，需要切换DRL模型
            if old_rsu != veh.current_rsu and new_rsu is not None and current_rsu_idx is not None:
                # 切换到新RSU的模型
                old_model = veh.model
                veh.model = copy.deepcopy(self.DRL_models[current_rsu_idx])
                veh.model.update_alpha(self.alpha.detach().numpy(), episode=self.episode)
                # 注意：任务队列保持不变，可以在新RSU继续卸载
    ...
    def destroyVeh(self):
        ...
        for veh in self.Vehicle.copy():
            # 车辆行驶出最后一个RSU就销毁（x >= 最后一个RSU覆盖范围结束位置）
            last_rsu_coverage_end = self.rsu_coverage[-1][1] if len(self.rsu_coverage) > 0 else self.maxRoadLen
            if veh.x >= last_rsu_coverage_end:
                # 计算penalty（未完成任务）
                if veh.queue:
                    # 修复：确保AOI_list不为空，避免nan
                    aoi_mean = np.mean(veh.AOI_list) if veh.AOI_list else 0.0
                ...
                # 标记为待销毁
                vehicles_to_destroy.append(veh)
        
        # 销毁车辆并清理内存
        for veh in vehicles_to_destroy:
            # 清理车辆的任务队列
            if hasattr(veh, 'queue') and veh.queue:
                veh.queue.clear()
                del veh.queue
        ...
        # 计算penalty（消失车辆对还存在车辆的影响）
        if self.Vehicle and penalty > 0:
            self.penalty += penalty / len(self.Vehicle)
    ...
    def getuploadPower(self,allPowerGain,uploadveh,uploadveh_channel=None):
        if uploadveh == 0:
            return [0]
        ...
        # 如果没有提供channel列表，使用默认的self.uploadveh_channel（向后兼容）
        if uploadveh_channel is None:
            uploadveh_channel = self.uploadveh_channel
        
        # 确保uploadveh_channel是列表且长度匹配
        if len(uploadveh_channel) != uploadveh:
            print(f"⚠️ Warning: uploadveh_channel length ({len(uploadveh_channel)}) != uploadveh ({uploadveh})")
            if len(uploadveh_channel) == 0:
                return [0]
            # 如果长度不匹配，截断或填充到正确长度
            if len(uploadveh_channel) > uploadveh:
                uploadveh_channel = uploadveh_channel[:uploadveh]
            else:
                # 如果长度不足，用最后一个值填充
                if len(uploadveh_channel) > 0:
                    uploadveh_channel = uploadveh_channel + [uploadveh_channel[-1]] * (uploadveh - len(uploadveh_channel))
                else:
                    return [0]
        
        # 确保uploadveh_channel是numpy数组
        uploadveh_channel = np.array(uploadveh_channel)
        
        G = a * uploadveh_channel * np.ones((uploadveh,uploadveh)) - a * uploadveh_channel * np.eye(uploadveh) \
                - uploadveh_channel * np.eye(uploadveh)
    ...
    def updateSINR(self):
        # ==================== 多RSU支持：分别计算每个RSU的干扰 ====================
        # 动态计算每个RSU内所有车辆的功率增益
        allPowerGain = {}  # {rsu_id: total_power_gain}
        Channel_list = np.zeros((len(self.Vehicle)))
        
        # 分别统计每个RSU的车辆
        rsu_vehicles = {}  # {rsu_id: [vehicles]}
        rsu_channels = {}  # {rsu_id: [channels]}
        
        # 初始化每个RSU的统计
        for i in range(1, self.num_rsu + 1):
            allPowerGain[i] = 0
            rsu_vehicles[i] = []
            rsu_channels[i] = []
        ...
        for veh in self.Vehicle:
            ...
            # 根据车辆当前所在的RSU，分别累加功率增益
            current_rsu = veh.current_rsu if hasattr(veh, 'current_rsu') else 1
            power_gain = veh.action * veh.channel
            
            if current_rsu in allPowerGain:
                allPowerGain[current_rsu] += power_gain
                rsu_vehicles[current_rsu].append(veh)
                rsu_channels[current_rsu].append(veh.channel)
        
        # 第二步：分别计算每个RSU的上传功率
        # 注意：这里计算的是用于模型上传的功率，不是任务卸载的功率
        # 在多RSU场景中，如果不需要模型上传，可以设置为0
        # 如果需要模型上传，应该按RSU分别计算
        uploadpower = {}  # {rsu_id: upload_power}
        for i in range(1, self.num_rsu + 1):
            uploadpower[i] = [0]  # 目前设置为0，不需要模型上传
       
        # 第三步：根据车辆所在的RSU，使用对应的干扰计算SINR
        veh_i = 0
        for veh in self.Vehicle:
            current_rsu = veh.current_rsu if hasattr(veh, 'current_rsu') else 1
            
            # 根据车辆所在的RSU，使用对应的干扰和上传功率
            if current_rsu in allPowerGain:
                veh.compute_sinr(allPowerGain[current_rsu], uploadpower[current_rsu][0])
            else:
                # 默认使用第一个RSU的干扰
                veh.compute_sinr(allPowerGain.get(1, 0), uploadpower.get(1, [0])[0])
    ...
    def updateMemory(self,Channel_list):
        ...
        for veh in self.Vehicle:
            ...
            # s1 = self.processState(s1)
            # 多RSU场景：车辆驶出最后一个RSU时标记为done
            last_rsu_coverage_end = self.rsu_coverage[-1][1] if len(self.rsu_coverage) > 0 else self.maxRoadLen
            if (veh.x + self.slotT * veh.v) >= last_rsu_coverage_end:
                veh.done = True
    ...
    def getWeight(self,veh,node_link):
        ...
        # 检查feature_mean是否过大（现在应该在0-1范围内）
        if feature_mean > 5.0:  # 如果feature_mean > 5，exp(5) ≈ 148，exp(10) ≈ 22026
            print(f"⚠️⚠️⚠️[DEBUG] getWeight: Large feature_mean detected! node_feature shape = {node_feature.shape}, feature_mean = {feature_mean}, exp_result = {exp_result}")
            import pdb
            pdb.set_trace()
        
        # 只在结果为NaN或Inf时打印
        if np.isnan(exp_result).any() or np.isinf(exp_result).any():
            print(f"⚠️⚠️⚠️[DEBUG] getWeight: NaN/Inf detected! node_feature shape = {node_feature.shape}, feature_mean = {feature_mean}, exp_result = {exp_result}")
            import pdb
            pdb.set_trace()
        ...
        for aggregate_node in aggregate_node_list:
            ...
            for veh2 in aggregate_veh_list:
                ...
                # 检查feature_mean是否过大（现在应该在0-1范围内）
                if feature_mean > 5.0:  # 如果feature_mean > 5，exp(5) ≈ 148，exp(10) ≈ 22026
                    print(f"⚠️⚠️⚠️[DEBUG] getWeight loop: Large feature_mean detected! node_feature shape = {node_feature.shape}, feature_mean = {feature_mean}, exp_result = {exp_result}")
                    import pdb
                    pdb.set_trace()
                
                # 只在结果为NaN或Inf时打印
                if np.isnan(exp_result).any() or np.isinf(exp_result).any():
                    print(f"⚠️⚠️⚠️[DEBUG] getWeight loop: NaN/Inf detected! node_feature shape = {node_feature.shape}, feature_mean = {feature_mean}, exp_result = {exp_result}")
                    import pdb
                    pdb.set_trace()
                weight.append(exp_result)
    ...
    def localAsyFederated(self):
        ...
        for veh in self.Vehicle:
            ...
            # 修复：防止除零错误，确保权重归一化安全
            if np.sum(mean_weight) != 0:
                mean_weight = (weight / np.sum(mean_weight)).tolist()
            else:
                mean_weight = [1.0] * len(weight)  # 默认均匀权重
    ...
    def globalAsyFederated(self):
        ...
        for i, node_feture in enumerate(self.G_conv):
            feature_mean = np.mean(np.abs(node_feture.detach().numpy()))
            exp_result = np.exp(feature_mean)
            
            # 检查feature_mean是否过大（现在应该在0-1范围内）
            if feature_mean > 5.0:  # 如果feature_mean > 5，exp(5) ≈ 148，exp(10) ≈ 22026
                print(f"⚠️⚠️⚠️[DEBUG] G_total_feature loop {i}: Large feature_mean detected! node_feture shape = {node_feture.shape}, feature_mean = {feature_mean}, exp_result = {exp_result}")
                import pdb
                pdb.set_trace()
            
            # 只在结果为NaN或Inf时打印
            if np.isnan(exp_result) or np.isinf(exp_result):
                print(f"⚠️⚠️⚠️[DEBUG] G_total_feature loop {i}: NaN/Inf detected! node_feture shape = {node_feture.shape}, feature_mean = {feature_mean}, exp_result = {exp_result}")
                import pdb
                pdb.set_trace()
            
            G_total_feature += exp_result
            
            # 检查累积结果
            if np.isnan(G_total_feature) or np.isinf(G_total_feature):
                print(f"⚠️⚠️⚠️[DEBUG] G_total_feature loop {i}: G_total_feature = {G_total_feature}")
                import pdb
                pdb.set_trace() 
        ...
        if len(self.Vehicle) > 0 :
            if self.GCN_model.GCN_memory.memory_counter > self.GCN_model.GCNupadate_counter :
                ...
                for it in range(self.GCN_model.GCN_Training_number):
                    # 确保有足够的数据进行采样
                    available_size = len(self.GCN_model.GCN_memory.memory_gcn)
                    if available_size < minibatch:
                        continue
                        
                    if available_size > self.GCN_model.GCN_memory.memory_size:
                        sample_index = np.random.choice(self.GCN_model.GCN_memory.memory_size, size=minibatch, replace=False)
                    else:
                        sample_index = np.random.choice(available_size, size=minibatch, replace=False)

                    # 从list中获取数据
                    Traindata_node = torch.stack([self.GCN_model.GCN_memory.memory_gcn[i] for i in sample_index])
                    r = [torch.tensor(self.GCN_model.GCN_memory.memory_r[i],dtype=torch.float32) for i in sample_index]
                    r = torch.stack(r).reshape(-1,1)
                    Traindata_node_ = torch.stack([self.GCN_model.GCN_memory.memory_gcn_[i] for i in sample_index])
                    ...
                    G_datax =  torch.cat([data.x  for data in G_data],dim = 0).view(minibatch,-1)
                    G_datax_ = torch.cat([data.x for data in G_data_], dim=0).view(minibatch, -1)

                    # 检查G_datax是否有异常值
                    if torch.isnan(G_datax).any() or torch.isinf(G_datax).any():
                        print("⚠️ ⚠️ Warning: NaN or Inf detected in G_datax, skipping this batch")
                        import pdb
                        pdb.set_trace()
                        continue
                    if torch.isnan(G_datax_).any() or torch.isinf(G_datax_).any():
                        print("⚠️ ⚠️ Warning: NaN or Inf detected in G_datax_, skipping this batch")
                        import pdb
                        pdb.set_trace()
                        continue

                    Convdata = torch.cat([self.GCN_model.forward(data.x, data.edge_index) for data in G_data],dim = 0).view(minibatch,-1)
                    Convdata_ = torch.cat([self.GCN_model.forward(data.x, data.edge_index) for data in G_data_],dim=0).view(minibatch,-1)
                    
                    # 检查Convdata是否有异常值
                    if torch.isnan(Convdata).any() or torch.isinf(Convdata).any():
                        print("⚠️ ⚠️ Warning: NaN or Inf detected in Convdata, skipping this batch")  
                        import pdb
                        pdb.set_trace()
                        continue
                    if torch.isnan(Convdata_).any() or torch.isinf(Convdata_).any():
                        print("⚠️ ⚠️ Warning: NaN or Inf detected in Convdata_, skipping this batch")
                        import pdb
                        pdb.set_trace()
                        continue
                    ...
                    # 检查最终的GCNdata是否有异常值
                    if torch.isnan(GCNdata).any() or torch.isinf(GCNdata).any():
                        print("⚠️ ⚠️ Warning: NaN or Inf detected in GCNdata, skipping this batch") 
                        import pdb
                        pdb.set_trace()
                        continue
                    if torch.isnan(GCNdata_).any() or torch.isinf(GCNdata_).any():
                        print("⚠️ ⚠️ Warning: NaN or Inf detected in GCNdata_, skipping this batch")
                        import pdb
                        pdb.set_trace()
                        continue
    ...
    def updateGCNMemory(self):
        # 准备GCN数据并检查是否有NaN值
        last_node_conv = torch.cat((self.last_node.view(-1),self.last_G_conv.view(-1)))
        current_node_conv = torch.cat((self.node.view(-1), self.G_conv.view(-1)))
        
        # 检查数据质量
        if (not (torch.isnan(last_node_conv).any() or torch.isinf(last_node_conv).any() or
                 torch.isnan(current_node_conv).any() or torch.isinf(current_node_conv).any() or
                 np.isnan(self.reward) or np.isinf(self.reward))):
            self.GCN_model.GCN_memory.Addremember(last_node_conv, -self.reward, current_node_conv, 
                                                 self.last_GCN_data, self.GCN_data)
        else:
            print("Warning: NaN or Inf detected in GCN training data, skipping storage")
            import pdb
            pdb.set_trace()
    ...
    def step(self):
        ...
        meanLoss = 0
        self.destroyAOI = 0
        return self.reward, mean_veh_reward_value, self.penalty, len(self.Vehicle), self.destroyAOI,gnnloss,gnnCriticloss
