...
class My_GCN(torch.nn.Module):
    ...
    def GCN_train_from_torch(self,Traindata_node,r,Traindata_node_):
        ...
        # 构建一个恒为1但保留梯度的表达式
        # 使用归一化技巧：x / x.sum() * constant，其中constant=1
        node_sum_all = Traindata_node.sum()  # 所有元素的和
        normalized_value = (node_sum_all / (node_sum_all + 1.0)) + (1.0 / (node_sum_all + 1.0))  # 恒等于1
        
        # 使用reward作为权重
        reward_weight = r.mean().detach()  # 获取reward的标量值（不计算梯度）
        
        # loss = 1 * reward_weight，但保留归一化过程以保证梯度流
        # 注意：r在Addremember时已经是-self.reward，所以这里不需要再加负号
        gcn_loss = normalized_value * reward_weight
    ...
    def train(self,num_nodes,node_feature_size,out_feature_size,reward_scale):
...
class VEC_env:
    ...
    def __init__(self,lane,vehGenRate,slotT,VehSpeed,rsuW,TaskGenRate,TaskSize,TransmissionRange,gridW,node_feature_size,out_feature
                 ,p,DRL_model,GCN_batchsize,maxP,slideW,w,k,penalty_factor,powerFactor,param_noise_var,GCN_factor, critic_noise_var , rateDecay,reward_scale,penalty_multiplier=1.0,plot=False):
        super().__init__()
        self.episode = 0
        self.slotT = slotT
        self.lane = lane
        ...
        self.GCN_model.train(4 * (int(self.maxRoadLen/self.gridW)),self.node_feature_size,self.out_feature_size,reward_scale)
        ...
        if len(self.Vehicle) > 0 :
            if self.GCN_model.GCN_memory.memory_counter > self.GCN_model.GCNupadate_counter :
                ...
                for it in range(self.GCN_model.GCN_Training_number):
                    ...
                    gcn_loss = self.GCN_model.GCN_train_from_torch(GCNdata,r,GCNdata_)
    ...
    def step(self):
        ...
        meanLoss = 0
        self.destroyAOI = 0
        return self.reward, mean_veh_reward_value, self.penalty, len(self.Vehicle), self.destroyAOI,gnnloss
