...
P_channel = np.array([
    [0.00000, 0.00000, 0.00000, 0.00000, 0.40000, 0.20000, 0.40000, 0.00000, 0.00000, 0.00000],
    [0.00000, 0.00000, 0.00000, 0.00000, 0.07407, 0.59259, 0.25926, 0.03704, 0.03704, 0.00000],
    [0.00000, 0.00000, 0.01460, 0.01460, 0.12774, 0.43431, 0.30292, 0.08758, 0.01825, 0.00000],
    [0.0, 0.0, 0.002129978700212998, 0.014039859601403989, 0.1199488005119949, 0.4785152148478516, 0.30114698853011473,0.06592934070659295, 0.016589834101658988, 0.0016999830001699985],
    [0.00000, 0.00000, 0.00140, 0.01603, 0.12163, 0.46814, 0.30301, 0.06872, 0.02001, 0.00106],
    [0.00004, 0.00012, 0.00164, 0.01556, 0.11317, 0.43598, 0.32278, 0.08389, 0.02510, 0.00172],
    [0.00001, 0.00018, 0.00138, 0.00997, 0.07702, 0.34020, 0.36391, 0.15078, 0.05390, 0.00265],
    [4.0000400004000046e-05, 0.0001800018000180002, 0.0005600056000560006, 0.004380043800438005, 0.03457034570345703,0.20363203632036322, 0.4197341973419734, 0.24742247422474226, 0.08538085380853809, 0.004100041000410005],
    [0.0, 0.0, 0.00011000110001100013, 0.0012900129001290014, 0.029800298002980032, 0.1725817258172582,0.4490044900449005, 0.27878278782787824, 0.06520065200652006, 0.0032300323003230033],
    [0.00000, 0.00000, 0.00000, 0.00000, 0.01220, 0.13415, 0.47764, 0.31910, 0.05691, 0.00000]
])
...
class VEC_veh:
    def __init__(self,lane,VehSpeed,slotT,rsuX,rsuY,dis,taskGenRate,TaskSize,TransmissionRange,node,model,record_veh_num,reward,seed=123):
        ...
        # CSMA不使用DRL模型，model参数保留用于兼容性但忽略
        
        self.collisions = 0        # 累计冲突次数
        self.backoff = 0           # 当前退避计数器
        self.tx_remaining = 0      # 剩余传输时隙
        self.is_transmitting = False  # 传输状态标志
        self.task_completed = False   # 任务完成标志
        self.start_backoff(cw_min, cw_max)
        TaskSize = 0
        if len(self.queue) >0:
    ...
    def start_backoff(self, cw_min, cw_max):
        """初始化退避计数器（二进制指数退避算法）"""
        cw = (2 ** self.collisions) * cw_min  # 计算竞争窗口
        cw = min(cw, cw_max)  # 不超过最大窗口
        self.backoff = random.randint(0, cw - 1)  # 随机退避值
    ...
    def _simulate_markov_step(self):
        """进行一次马尔可夫链状态转移"""
        # 按照转移概率选择下一个状态
        # print(self.current_state_channel)
        # print(self.P_channel[self.current_state_channel])
        # print(sum(self.P_channel[self.current_state_channel]))
        next_state = np.random.choice(self.n_states_channel, p=self.P_channel[self.current_state_channel])
        self.current_state_channel = next_state
        # 生成当前状态对应的增益值
        self.current_gain_estimation = self._get_gain_from_state(next_state)
    ...
    def compute_sinr(self, allPowerGain, uploadmodelPower):
        ...
        self.sinr = self.action * self.channel / (
                    allPowerGain - self.action * self.channel + noise_var + uploadmodelPower)
...
class VEC_env:
        ...
        for lane_idx in range(self.lane):
            if self.GenNextSlot[lane_idx] <= 0:
                ...
                veh = VEC_veh(lane_idx, Speed, self.slotT, self.rsuX, self.rsuY, dis, self.TaskGenRate, self.TaskSize,
                              self.TransmissionRange, node, None, self.record_veh_num,
                              reward=self.reward)

                veh.p = min(np.random.random()+0.01,0.99)
                self.Vehicle.append(veh)
    ...
    def updateSINR(self):
        ...
        for veh in self.Vehicle:
            veh.Power_ = 0
            # print(veh," 有几个任务", len(veh.queue))
            for task in veh.queue:
                task.AOI += 1

        for veh in self.transmitting_vehs:
            veh.Translate = False
            veh.tx_remaining -= 1
            # 一旦有发送机会，车辆就发送任务
            veh.action = self.maxP
            First_task = veh.queue[0]

            if veh.tx_remaining <= 0:  # 传输完成
                veh.task_completed = True
                veh.collisions = 0  # 重置冲突计数器
                del First_task
                del veh.queue[0]
                self.transmitting_vehs.remove(veh)
                veh.is_transmitting = False
            else:
                remaining_vehs.append(veh)
        # self.transmitting_vehs = remaining_vehs


        # 阶段2：信道状态检测
        channel_idle = (len(self.transmitting_vehs) == 0)
        # 阶段3：处理退避过程
        candidates = []
        for veh in self.Vehicle:

            if len(veh.queue) == 0 or veh.is_transmitting:
                continue
            # print(veh.backoff)
            # print("###########################################")
            # 仅在信道空闲时递减退避计数器
            if channel_idle and veh.backoff > 0:
                veh.backoff -= 1

            if veh.backoff == 0:  # 退避完成
                candidates.append(veh)
        # print(candidates)
        # 阶段4：处理信道竞争
        if channel_idle and candidates:
            if len(candidates) == 1:  # 成功获得信道
                veh = candidates[0]
                veh.is_transmitting = True

                First_task = veh.queue[0]
                veh.action = self.maxP
                allPowerGain += veh.action * veh.channel
                # uploadpower = self.getuploadPower(allPowerGain, self.uploadveh)
                veh.compute_sinr(allPowerGain, 0)

                veh.tx_remaining = math.ceil(First_task.size / veh.v2iRate/0.02)
                # print(veh, " 任务大小 : ",First_task.size," 开始发送时间为 ： ", veh.tx_remaining)


                veh.Power_ = (First_task.size / veh.v2iRate) * veh.action

                self.thoughout = First_task.size
                self.transmitting_vehs.append(veh)

            else:  # 发生冲突
                for veh in candidates:
                    veh.collisions += 1
                    veh.start_backoff(cw_min, cw_max)

        # print("正在发送:",self.transmitting_vehs)

        for veh in self.Vehicle:
            veh.collectAOI()
            # print(veh, " backoff : ", veh.backoff, ' num task : ', len(veh.queue))
        self.reward ,self.slideReward = self.calculateAOI(0, 1)
        ...
        for veh in self.Vehicle:
            ...
            if len(veh.queue)>0:
                ...
                if veh.v2iRate >= First_task.size:
                    # print("传输功率: ", veh.action, "传输时间: ", First_task.size/veh.v2iRate/0.02)
                    self.thoughout += veh.v2iRate
                    veh.Translate = True
    ...
    def step(self):
        ...
        mean_veh_reward_list = []

        n_veh = len(self.Vehicle)
        for veh in self.Vehicle:
            mean_veh_reward_list.append(veh.reward)

        # 计算平均车辆奖励
        if len(mean_veh_reward_list) > 0:
            mean_veh_reward = np.mean(mean_veh_reward_list)
        else:
            mean_veh_reward = 0.0
        
        # 计算destroyAOI（已销毁车辆的任务数）
        destroyAOI = len(self.destroyAOI) if hasattr(self, 'destroyAOI') else 0
        
        # 返回与调用者接口匹配的值：AOI, mean_veh_reward, penalty, n_veh, destroyAOI, gnnloss, gnnCriticloss
        # CSMA不使用GNN，所以最后两个值设为0
        return self.reward, mean_veh_reward, self.penalty, n_veh, destroyAOI, 0.0, 0.0
