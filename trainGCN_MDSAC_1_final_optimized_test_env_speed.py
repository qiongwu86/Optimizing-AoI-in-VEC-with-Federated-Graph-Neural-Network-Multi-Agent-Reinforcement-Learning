...
def main():
    ...
    VehSpeed_base = [30, 35, 40, 50]  # 基础速度限速, km/h
    ...
    # ==================== 速度变化配置 ====================
    # 外层循环：遍历不同的速度变化百分比（±5%, ±10%, ±15%, ±20%, ±25%）
    # speed_variations = [5, 10, 15, 20, 25]  # 速度变化百分比
    speed_variations = [15]  # 速度变化百分比
    # ==================== 算法配置 ====================
    # 内层循环：遍历不同的算法
    algorithms = {
        'GNN_No_CRITC_SAC': {
            'name': 'GNN_No_CRITC_SAC',
            'model_path': '/Users/cz/Desktop/FinalConcise_test/训练结果2025101102/学习率0.9/对比算法/无gnn critic /MyCode1_p_0.1',
            'DRL_hidden_size': 64,
            'GCN_critic': ConcatMlp
        },
        # 'LFSAC': {
        #     'name': 'LFSAC',
        #     'model_path': '/Users/cz/Desktop/FinalConcise_test/训练结果2025101102/学习率0.9/对比算法/FL平均/MyCode1_p_0.1',
        #     'DRL_hidden_size': 64,
        #     'GCN_critic': ConcatMlp
        # }
        # 'FGNN_MASAC': {
        #     'name': 'FGNN_MASAC',
        #     'model_path': '/Users/cz/Desktop/FinalConcise_test/训练结果2025101102/学习率0.9/模型大小/64/MyCode1_p_0.1',
        #     'DRL_hidden_size': 64,
        #     'GCN_critic': ConcatMlp
        # }
    }
    ...
    # ==================== 外层循环：遍历速度变化 ====================
    for speed_var_percent in speed_variations:
        print("\n" + "=" * 80)
        print(f"Starting tests for Speed Variation: ±{speed_var_percent}%")
        print("=" * 80)
        
        # 注意：VehSpeed保持不变（作为限速），车辆速度会在限速基础上波动±speed_var_percent%
        # 不修改VehSpeed，而是在环境初始化时传递speed_variation_percent参数
        VehSpeed = VehSpeed_base
        print(f"Speed Limit (VehSpeed): {VehSpeed} km/h")
        print(f"Vehicle speed will vary within ±{speed_var_percent}% of the speed limit")
        ...
        for alg_key, alg_config in algorithms.items():
            ...
            print(f"Speed Variation: ±{speed_var_percent}%")
            print("=" * 80)
            ...
            run_algorithm_test(
                ...
                GCN_critic_class=GCN_critic_class, speed_var_percent=speed_var_percent
            )
...
def run_algorithm_test(
    ...
    DRL_hidden_size, algorithm_name, model_base_path, GCN_critic_class, speed_var_percent
):
    """运行单个算法的测试"""
    ...
    env = VEC_env(lane = num_lane,vehGenRate = vehGenRate,
                  ...
                  speed_variation_percent=speed_var_percent  # 传递速度波动百分比参数
                  )
        ...
        try:
            ...
            if os.path.exists(trained_DRL_model_path):
                ...
                if 'model_state_dict' in checkpoint:
                    # 使用ModelOptimizer.save_model_optimized()保存的格式（最常见）
                    print("   Using 'model_state_dict' format...")
                    env.DRL_model.load_state_dict(checkpoint['model_state_dict'])
                elif 'policy' in checkpoint:
                    # 使用DRL_SAC.state_dict()保存的格式
                    print("   Using 'policy/qf1/qf2' format...")
                    env.DRL_model.load_state_dict(checkpoint)
                elif 'actor' in checkpoint:
                    # 兼容旧格式（使用actor和critic键名）
                    print("   Using legacy 'actor/critic' format...")
                    ...
                    # 尝试直接加载为完整的state_dict
                    print("   Trying to load as direct state_dict...")
                    try:
                        env.DRL_model.load_state_dict(checkpoint)
                    except Exception as load_error:
                        print(f"   ⚠️ 无法识别模型格式: {load_error}")
                        raise
    ...
    # ==================== 测试数据保存配置 ====================
    # 创建保存目录结构：/Test/speed/{speed_var_percent}%/{algorithm_name}/
    test_data_base_path = f"/Users/cz/Desktop/FinalConcise_test/Test/speed/{speed_var_percent}%/{algorithm_name}"
    ...
    with open(summary_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f"Speed Variation: ±{speed_var_percent}%\n")
