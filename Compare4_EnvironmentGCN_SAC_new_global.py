...
class VEC_env:
    ...
    def step(self):
        ...
        # self.GenNextSlot -= 1
        if self.Vehicle[0].model.buffer.current_size >= self.args.batch_size:
            for veh in self.Vehicle:
                transitions = veh.model.buffer.sample(self.args.batch_size)
                veh.model.learn(transitions, None)  # 需要自己的和其他的
