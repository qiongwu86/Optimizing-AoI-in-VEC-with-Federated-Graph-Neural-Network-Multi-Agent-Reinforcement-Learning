...
class Buffer:
    def __init__(self, args):
        ...
        if self.args.type == 0:
            ...
            self.buffer['o'] = np.empty([self.size, self.args.obs_shape[0]])
            self.buffer['u'] = np.empty([self.size, self.args.action_shape[0]])
            self.buffer['r'] = np.empty([self.size])
            self.buffer['o_next'] = np.empty([self.size, self.args.obs_shape[0]])
        # thread lock
        if args.type == 0:
