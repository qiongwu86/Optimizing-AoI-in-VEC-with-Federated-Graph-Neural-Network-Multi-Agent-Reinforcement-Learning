...
def main():
    ...
    vehGenRate = [8, 8, 8, 8]
    TaskSize_list = [5, 10, 15, 20, 25]  # 性能 大小----------------------------------------------------------------------------------------------------------------
    p = 0.0 # 性能
    GCN_batchsize = 64
    ...
    for algo_key, algo_config in algorithms.items():
        ...
        DRL_hidden_size = algo_config['DRL_hidden_size']
        
        # ==================== 第二层循环：TaskSize ====================
        for TaskSize in TaskSize_list:
            print("\n" + "=" * 80)
            print(f"Algorithm: {algo_name}, TaskSize: {TaskSize}")
            print("=" * 80)
            ...
            env = VEC_env(lane = num_lane,vehGenRate = vehGenRate,
                          ...
                          rateDecay = rateDecay,
                          GCN_critic = algo_config['GCN_critic'],
                          reward_scale=reward_scale,
            ...
            # ==================== 模型加载配置 ====================
            # 训练好的模型路径（根据算法配置）
            trained_model_base_path = algo_config['model_path']
            ...
            # ==================== 测试数据保存配置 ====================
            test_data_base_path = f"/Users/wang/Documents/研究生资料/自己论文/第二篇/FinalConcise_20251103/Test/methods/{TaskSize}"
            ...
            print("\n" + "=" * 60)
            print(f"Starting {num_runs} test runs for {algo_name} with TaskSize={TaskSize}...")
            print("=" * 60)
            
            # ==================== 第三层循环：运行次数 ====================
            for run_idx in range(num_runs):
                print(f"\n{'='*60}")
                print(f"Run {run_idx + 1}/{num_runs} - Algorithm: {algo_name}, TaskSize: {TaskSize}")
                print(f"{'='*60}")
                ...
                for episode in range(TotalEpi):
                    ...
                    for simSlot in range(int(TotalSlot)):
                        print(f"Algorithm: {algo_name}, TaskSize: {TaskSize}, Run {run_idx + 1}/{num_runs} - episode: {episode}, slot: {simSlot}, vehicle number: {len(env.Vehicle)}")
            ...
            # ==================== 计算平均值、标准差和置信区间 ====================
            print("\n" + "=" * 60)
            print(f"Computing statistics (mean, std, confidence intervals) for {algo_name} with TaskSize={TaskSize}...")
            ...
            with open(summary_file, 'w', encoding='utf-8') as f:
                f.write("=" * 60 + "\n")
                f.write(f"Test Summary Report (Statistics from {num_runs} Runs)\n")
                f.write(f"Algorithm: {algo_name}, TaskSize: {TaskSize}\n")
                f.write("=" * 60 + "\n")
            ...
            print("\n" + "=" * 60)
            print(f"Test Summary (Statistics from {num_runs} Runs)")
            print(f"Algorithm: {algo_name}, TaskSize: {TaskSize}")
            print("=" * 60)
            ...
            print(f"\nCompleted testing for {algo_name} with TaskSize={TaskSize}.\n")
