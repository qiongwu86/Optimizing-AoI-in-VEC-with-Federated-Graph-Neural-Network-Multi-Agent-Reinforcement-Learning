...
from EnvironmentGCN_SAC_new_global2_test_env_channel import  VEC_env
...
def main():
    ...
    DRL_hidden_size = 64  # 可以设置为 64, 128, 或 256
    
    # ==================== CSI滞后时间配置 ====================
    # 外层循环：遍历不同的CSI滞后时间
    csi_lag_slots_list = [1, 5, 10, 15, 20, 25]
    # csi_lag_slots_list = [10]
    # CSI噪声方差配置（固定值）
    csi_noise_variance_db = -20  # 可以设置为 -20, -10, 或 -5
    # 是否启用CSI陈旧/噪声
    enable_csi_staleness = True  # 设置为False可以禁用此功能
    ...
    algorithms = {
        ...
        #     'model_path': '/Users/macchuzu/Desktop/FinalConcise_M4test/训练结果2025101102/学习率0.9/对比算法/无gnn critic /MyCode1_p_0.1',
        #     'DRL_hidden_size': 64,
        #     'GCN_critic': ConcatMlp  # 仍然使用ConcatMlp，但模型训练时没有GCN critic
        # }
        # 'LFSAC': {
        #     'name': 'LFSAC',
        #     'model_path': '/Users/macchuzu/Desktop/FinalConcise_M4test/训练结果2025101102/学习率0.9/对比算法/FL平均/MyCode1_p_0.1',
        #     'DRL_hidden_size': 64,
    ...
    # ==================== 外层循环：遍历CSI滞后时间 ====================
    for csi_lag_slots in csi_lag_slots_list:
        print("\n" + "=" * 80)
        print(f"Starting tests for CSI Lag Slots = {csi_lag_slots}")
        print("=" * 80)
        ...
        for alg_key, alg_config in algorithms.items():
            ...
            print(f"CSI Lag Slots: {csi_lag_slots}")
            print("=" * 80)
            ...
            run_algorithm_test(
                ...
                csi_lag_slots=csi_lag_slots, csi_noise_variance_db=csi_noise_variance_db,
                enable_csi_staleness=enable_csi_staleness,
                algorithm_name=algorithm_name, model_base_path=model_base_path,
                GCN_critic_class=GCN_critic_class
            )
...
def run_algorithm_test(
    ...
    DRL_hidden_size, csi_lag_slots, csi_noise_variance_db, enable_csi_staleness,
    algorithm_name, model_base_path, GCN_critic_class
):
    """运行单个算法的测试"""
    ...
    env = VEC_env(lane = num_lane,vehGenRate = vehGenRate,
                  ...
                  csi_lag_slots=csi_lag_slots,
                  csi_noise_variance_db=csi_noise_variance_db,
                  enable_csi_staleness=enable_csi_staleness
                  )
    ...
    pdb.set_trace()
    # ==================== 测试数据保存配置 ====================
    # 创建保存目录结构：/Test/channel/{csi_lag_slots}/{algorithm_name}/
    test_data_base_path = f"/Users/wang/Documents/研究生资料/自己论文/第二篇/FinalConcise_20251103/Test/channel/{csi_lag_slots}"
    ...
    with open(summary_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f"CSI Lag Slots: {csi_lag_slots}\n")
        ...
        f.write(f"  CSI Lag Slots: {csi_lag_slots}\n")
        f.write(f"  CSI Noise Variance (dB): {csi_noise_variance_db}\n")
        f.write("=" * 60 + "\n")
    
    print("\n" + "=" * 60)
    print(f"Test Summary for {algorithm_name} (CSI Lag Slots = {csi_lag_slots})")
    print("=" * 60)
    ...
    print(f"\n✅ Test completed for {algorithm_name} with CSI Lag Slots = {csi_lag_slots}.")
