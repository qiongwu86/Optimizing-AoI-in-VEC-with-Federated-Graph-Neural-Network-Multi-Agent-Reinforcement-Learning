...
from EnvironmentGCN_SAC_new_global2_local_fed import  VEC_env
...
def main():
    ...
    env = VEC_env(lane = num_lane,vehGenRate = vehGenRate,
                  ...
                  DRL_model=  DRL_model,p = p,maxP =maxP,slideW = slideW,w = w,k=k,
    ...
    for episode in range(TotalEpi):
        with memory_optimizer.memory_context(f"Episode {episode}"):
            ...
            for simSlot in range(int(TotalSlot)):
                ...
                # 使用内存监控的训练步骤
                AOI, mean_veh_reward ,penalty,  n_veh, destroyAOI = memory_optimizer.monitor_training_step(
                    env.step
                )
