...
class VEC_veh:
    def __init__(self,lane,x,VehSpeed,slotT,rsuX,rsuY,dis,taskGenRate,TaskSize,TransmissionRange,node,model,record_veh_num,reward,seed=123):
        ...
        self.action = self.model.select_action(self.s0,0.1,0.1)
        self.action_pre = 0
...
class VEC_env:
    ...
    def generateVeh(self):
        ...
        if len(self.Vehicle) == 0:
            ...
            for id, agent in enumerate(self.DRL_model):
                self.record_veh_num += 1
                ...
                veh = VEC_veh(lane_idx, x, Speed, self.slotT, self.rsuX, self.rsuY, dis, self.TaskGenRate, self.TaskSize,
                              self.TransmissionRange, node, copy.deepcopy(agent), self.record_veh_num,
                              reward=self.reward)
    ...
    def updateSINR(self):
        ...
        for veh in self.Vehicle:
            ...
            action = veh.model.select_action(veh.s0,self.noise,self.epsilon)
            # _, action, appro_action = veh.model.select_action(veh.s0,self.maxP, veh.record_id,len(self.Vehicle))
    ...
    def updateMemory(self,Channel_list):
        ...
        for veh in self.Vehicle:
            ...
            s.append(np.array(veh.s0))
            veh.s0 = s1
            u.append(np.array(veh.action))
            r.append(np.array(-veh.reward))
            s_next.append(np.array(s1))
    ...
    def step(self):
        ...
        # self.GenNextSlot -= 1
        if self.buffer.current_size >= self.args.batch_size  and (self.slot % 100 == 0):
            agent_all = []
            for veh in self.Vehicle:
                agent_all.append(veh.model)
            transitions = self.buffer.sample(self.args.batch_size)
            for id, agent in enumerate(agent_all):
                other_agents = agent_all.copy()
                other_agents.remove(agent)
                agent.learn(transitions, other_agents)  # 需要自己的和其他的
                #print(" --> Train veh : ", id)
    ...
    def printVeh(self):
        ...
        for veh in self.Vehicle:
           print(
                 # "slot : ", self.slot,
                 "ID : ",veh.record_id,
                 " , AOI : %.2f" % (np.mean(veh.AOI_list) if len(veh.AOI_list) > 0 else 0.00),
                 " , reward : % .2f" % veh.reward,
                 " , power : ",veh.action,
                 # " Training number : " , veh.trainNumber,
                 # "local aggregation : " ,veh.n_LocalDRL ,
                 # "glabal aggregation : ",veh.n_GlobalDRL,
