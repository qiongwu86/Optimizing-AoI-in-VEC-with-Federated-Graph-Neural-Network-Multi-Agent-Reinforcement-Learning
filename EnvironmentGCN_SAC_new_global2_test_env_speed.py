...
class VEC_veh:
    def __init__(self,lane,VehSpeed,slotT,rsuX,rsuY,dis,taskGenRate,TaskSize,TransmissionRange,node,model,record_veh_num,reward,seed=123):
        ...
        self.v = VehSpeed
        # 变速运动相关属性
        self.desired_v = VehSpeed  # 期望速度（初始为车道速度）
        self.acceleration = 0.0  # 当前加速度 (m/s^2)
        self.max_acceleration = 2.0  # 最大加速度 (m/s^2)
        self.max_deceleration = -3.0  # 最大减速度 (m/s^2)
        self.min_safe_distance = 5.0  # 最小安全间距 (m)
        self.reaction_time = 0.5  # 反应时间 (s)
...
class VEC_env:
    ...
    def __init__(self,lane,vehGenRate,slotT,VehSpeed,rsuW,TaskGenRate,TaskSize,TransmissionRange,gridW,node_feature_size,out_feature
                 ,p,DRL_model,GCN_batchsize,maxP,slideW,w,k,penalty_factor,powerFactor,param_noise_var,GCN_factor, critic_noise_var , rateDecay,GCN_critic,reward_scale,penalty_multiplier=1.0,speed_variation_percent=0.0,plot=False):
        super().__init__()
        self.episode = 0
        self.slotT = slotT
        self.lane = lane
        ...
        self.speed_variation_percent = speed_variation_percent  # 速度波动百分比（例如：5表示±5%）
    ...
    def resetVeh(self,episode):
        ...
        for veh in self.Vehicle:
            ...
            # 计算基础速度（从km/h转换为m/s）
            base_speed = self.VehSpeed[lane_idx[veh_i]] * 1000 / 3600
            # 应用速度波动：在限速基础上随机波动 ±speed_variation_percent%
            if self.speed_variation_percent > 0:
                variation_factor = 1.0 + np.random.uniform(-self.speed_variation_percent/100.0, self.speed_variation_percent/100.0)
                Speed = base_speed * variation_factor
            else:
                Speed = base_speed
            ...
            veh.VehSpeed = Speed
            veh.v = Speed  # 更新当前速度
            veh.desired_v = Speed  # 更新期望速度
            veh.acceleration = 0.0  # 重置加速度
            veh.dis = dis
            veh.node = node
    ...
    def generateVeh(self):
        ...
        for lane_idx in range(self.lane):
            if self.GenNextSlot[lane_idx] <= 0:
                ...
                # 计算基础速度（从km/h转换为m/s）
                base_speed = self.VehSpeed[lane_idx] * 1000 / 3600
                # 应用速度波动：在限速基础上随机波动 ±speed_variation_percent%
                if self.speed_variation_percent > 0:
                    variation_factor = 1.0 + np.random.uniform(-self.speed_variation_percent/100.0, self.speed_variation_percent/100.0)
                    Speed = base_speed * variation_factor
    ...
    def updateVehLoc(self):
        # 按车道和位置对车辆进行排序，以便找到前车
        # 对每个车道的车辆按x坐标排序（从小到大，即从前到后）
        vehicles_by_lane = {}
        for veh in self.Vehicle:
            if veh.lane not in vehicles_by_lane:
                vehicles_by_lane[veh.lane] = []
            vehicles_by_lane[veh.lane].append(veh)
        
        # 对每个车道的车辆按x坐标排序（从小到大）
        for lane in vehicles_by_lane:
            vehicles_by_lane[lane].sort(key=lambda v: v.x)
        
        # 更新每辆车的位置和速度
        for veh in self.Vehicle:
            # 找到同车道的前车（x坐标更大的车辆，即在前面的车辆）
            front_vehicle = None
            if veh.lane in vehicles_by_lane:
                lane_vehicles = vehicles_by_lane[veh.lane]
                # 找到在当前车辆前面的车辆（x坐标更大且最接近的）
                min_distance = float('inf')
                for fv in lane_vehicles:
                    if fv.x > veh.x:
                        distance = fv.x - veh.x
            ...
            if front_vehicle is not None:
                distance_to_front = front_vehicle.x - veh.x
                front_vehicle_speed = front_vehicle.v
            else:
                # 没有前车，可以自由加速到期望速度
                distance_to_front = float('inf')
                front_vehicle_speed = veh.desired_v
            
            # 计算期望的安全间距（基于当前速度和前车速度）
            # 安全间距 = 最小安全间距 + 反应时间内的行驶距离 + 速度差导致的额外间距
            relative_speed = veh.v - front_vehicle_speed if front_vehicle is not None else 0
            safe_distance = veh.min_safe_distance + veh.reaction_time * veh.v + \
                           max(0, relative_speed * veh.reaction_time)
            
            # 计算加速度（简化的跟车模型）
            if front_vehicle is None:
                # 没有前车，加速到期望速度
                speed_diff = veh.desired_v - veh.v
                if speed_diff > 0:
                    # 加速
                    veh.acceleration = min(veh.max_acceleration, speed_diff / self.slotT)
                else:
                    # 减速（如果当前速度超过期望速度）
                    veh.acceleration = max(veh.max_deceleration, speed_diff / self.slotT)
            else:
                # 有前车，根据距离和速度差调整加速度
                if distance_to_front < safe_distance:
                    ...
                    if relative_speed > 0:
                        # 如果速度比前车快，需要减速
                        decel_needed = (relative_speed ** 2) / (2 * max(distance_to_front - veh.min_safe_distance, 1.0))
                        veh.acceleration = max(veh.max_deceleration, -decel_needed)
                    else:
                        # 速度比前车慢或相同，可以轻微加速或保持
                        veh.acceleration = min(veh.max_acceleration * 0.5, 
                                             (distance_to_front - safe_distance) / self.slotT)
                elif distance_to_front > safe_distance * 1.5:
                    # 距离较远，可以加速到期望速度
                    speed_diff = min(veh.desired_v, front_vehicle_speed) - veh.v
                    if speed_diff > 0:
                        veh.acceleration = min(veh.max_acceleration, speed_diff / self.slotT)
                    else:
                        veh.acceleration = 0
                    ...
                    if relative_speed > 0.1:
                        # 稍微减速
                        veh.acceleration = max(veh.max_deceleration * 0.5, -relative_speed / self.slotT)
                    elif relative_speed < -0.1:
                        # 稍微加速
                        veh.acceleration = min(veh.max_acceleration * 0.5, -relative_speed / self.slotT)
                    else:
                        # 保持速度
                        veh.acceleration = 0
            
            # 限制加速度在合理范围内
            veh.acceleration = np.clip(veh.acceleration, veh.max_deceleration, veh.max_acceleration)
            
            # 更新速度
            new_v = veh.v + veh.acceleration * self.slotT
            # 确保速度不为负
            veh.v = max(0, new_v)
            # 限制速度不超过期望速度的1.2倍（安全限制）
            veh.v = min(veh.v, veh.desired_v * 1.2)
            
            # 更新位置
            veh.x += veh.v * self.slotT + 0.5 * veh.acceleration * (self.slotT ** 2)
