...
from EnvironmentGCN_SAC_test2 import  VEC_env
...
def main():
    ...
    # VehSpeed = [30, 30, 30, 30]
    # VehSpeed = [40, 40, 40, 40]
    # VehSpeed = [50, 50, 50, 50]
    VehSpeed = [60, 60, 60, 60]
    # VehSpeed = [70, 70, 70, 70]
    # VehSpeed = [80, 80, 80, 80]
    "----------------------------------------"
    ...
    DRL_dir = dir + "SAC/" 
    data_dir = "./save/Test/MyCode_size1_speed/4/"
    ...
    # plt.show()
    #
    np.save(data_dir+  "AOI.npy",   np.mean(AOE_T))
    np.save(data_dir + "power.npy", np.mean(Power_T))
    np.save(data_dir + "Queue.npy", np.mean(Queue_T))
    np.save(data_dir + "n_veh.npy", np.mean(N_veh_T))
    np.save(data_dir + "thoughout.npy", np.mean(thoughout_T))
