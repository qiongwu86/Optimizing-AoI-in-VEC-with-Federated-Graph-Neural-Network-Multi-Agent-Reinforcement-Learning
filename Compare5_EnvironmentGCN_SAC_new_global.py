...
class VEC_veh:
    def __init__(self,lane,x,VehSpeed,slotT,rsuX,rsuY,dis,taskGenRate,TaskSize,TransmissionRange,node,model,record_veh_num,reward,seed=123):
        super().__init__()
        ...
        "random"
        self.action = random.uniform(0,20)

        # self.action_pre = 0
        # self.appro_action = self.model.select_action(self.s0,0.1,0.1)
    ...
    def reset(self,model):
        ...
        # self.s1      = np.concatenate(([0], [0], [0],[TaskSize],[0],[self.appro_action]))
        self.s1 = np.concatenate(([0], [0], [0], [0], [TaskSize], [0]))
        self.action = self.model.select_action(self.s0,self.record_id)
        self.action_pre = 0
...
class VEC_env:
    ...
    def __init__(self,lane,vehGenRate,slotT,VehSpeed,rsuW,TaskGenRate,TaskSize,TransmissionRange,gridW,node_feature_size,out_feature
                 ,p,DRL_model,GCN_batchsize,maxP,slideW,w,k,penalty_factor,powerFactor,param_noise_var,GCN_factor,
                 critic_noise_var , rateDecay,GCN_critic,reward_scale,args,plot=False):
        super().__init__()
        self.args = args
        self.episode = 0
        self.slotT = slotT
    ...
    def func(self,pop):
        ...
        "上传模型的功率"
        # uploadpower = self.getuploadPower(allPowerGain,self.uploadveh) # 上传模型的功率
        veh_i = 0
        for veh in self.Vehicle:
        ...
        self.reward ,fitness = self.Translate_task()
        return fitness

    def initPSO(self):
        self.pso_w = getweight()
        self.pso_lr = getlearningrate()
        self.pso_maxgen = getmaxgen()
        self.pso_sizepop = getsizepop(self.args.n_agents)
        self.pso_rangepop = getrangepop()
        self.pso_rangespeed = getrangespeed()
    ...
    def generateVeh(self):
        ...
        if len(self.Vehicle) == 0:
            ...
            for id in range(self.args.n_agents):
                self.record_veh_num += 1
                lane_idx = random.choice([0,1,2,3])

                Speed = self.VehSpeed[lane_idx] * 1000 / 3600
                " x < rsuW "
                x_random =  random.uniform(0,200)

                while x_random in all_x[lane_idx]:
                    x_random = random.uniform(0, 200)
                all_x[lane_idx].append(x_random)
                x = x_random

                dis = np.sqrt((self.rsuX - x) ** 2 + (self.rsuY - 3.5 / 2 - lane_idx * 3.5) ** 2 + Hight_RSU ** 2)
                node = int(lane_idx * int(self.maxRoadLen / self.gridW) + 0 // self.gridW)

                veh = VEC_veh(lane_idx, x, Speed, self.slotT, self.rsuX, self.rsuY, dis, self.TaskGenRate, self.TaskSize,
            ...
            for veh in self.Vehicle:
                x_random = random.uniform(0, 200)
                while x_random in all_x[veh.lane]:
                    x_random = random.uniform(0, 200)
                all_x[veh.lane].append(x_random)
                x = x_random
                veh.x = x
        # print(0)
    ...
    def calculateAOI(self,Transimission_AOI,Transimission_veh):
        self.meanAOI = 0
        fitness = np.zeros(self.pso_sizepop)
        ...
        for veh in self.Vehicle:
            ...
            if veh.queue:
                if veh.Transimission_AOI is not None:
                    ...
                    fitness[veh_i] = np.mean(veh.AOI_list) + veh.Transimission_AOI
                else:
                    self.meanAOI += np.mean(veh.AOI_list)
                    fitness[veh_i] = np.mean(veh.AOI_list)
            veh_i+=1
        if len(self.Vehicle)>0:
            self.meanAOI = self.meanAOI/len(self.Vehicle)
        return self.meanAOI,fitness
    ...
    def updateSINR(self):
        ...
        self.pso_pop, self.pso_v, _ = initpopvfit(self.pso_sizepop,self.pso_rangepop)
        self.pso_fitness = self.func(self.pso_pop)
        self.pso_gbestpop, self.pso_gbestfitness, self.pso_pbestpop, self.pso_pbestfitness = getinitbest(self.pso_fitness, self.pso_pop)
        self.pso_result = np.zeros(self.pso_maxgen)

        for i in range(self.pso_maxgen):
            t = 0.5
            # 速度更新
            for j in range(self.pso_sizepop):
                self.pso_v[j] += self.pso_lr[0] * np.random.rand() * (self.pso_pbestpop[j] - self.pso_pop[j]) + self.pso_lr[1] * np.random.rand() * (
                            self.pso_gbestpop - self.pso_pop[j])
            self.pso_v[self.pso_v < self.pso_rangespeed[0]] = self.pso_rangespeed[0]
            self.pso_v[self.pso_v > self.pso_rangespeed[1]] = self.pso_rangespeed[1]

            # 粒子位置更新
            for j in range(self.pso_sizepop):
                # pop[j] += 0.5*v[j]
                self.pso_pop[j] = t * (0.5 * self.pso_v[j]) + (1 - t) * self.pso_pop[j]
            self.pso_pop[self.pso_pop < self.pso_rangepop[0]] = self.pso_rangepop[0]
            self.pso_pop[self.pso_pop > self.pso_rangepop[1]] = self.pso_rangepop[1]
            ...
            for j in range(self.pso_sizepop):
                if self.pso_fitness[j] > self.pso_pbestfitness[j]:
                    self.pso_pbestfitness[j] = self.pso_fitness[j]
                    self.pso_pbestpop[j] = self.pso_pop[j].copy()
    ...
    def Translate_task(self):
        ...
        mean_AOI , fitness = self.calculateAOI(Transimission_AOI,Transimission_veh)

        return mean_AOI, fitness
    ...
    def updateMemory(self,Channel_list):
        ...
        for veh in self.Vehicle:
            ...
            veh.model.buffer.store_episode(np.array(veh.s0), [np.array(veh.action)], [np.array(-veh.reward)], np.array(s1))
            veh.s0 = s1
    ...
    def step(self):
        ...
        return self.reward, np.mean(mean_veh_reward) , self.penalty, len(self.Vehicle), self.exceed , 0, 0
